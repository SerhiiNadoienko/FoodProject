...
function calc() {
  ...
  let sex, height, weight, age, ratio; //проверки на локал стореж + ставим дефолтное значение
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); //проверки на локал стореж + ставим дефолтное значение
  ...
  calcTotal(); //функция по получению информации со статических элементов 

  function getStaticInformation(selector, activeClass) {
    const elements = document.querySelectorAll(selector); //вешаем на все элементы обработчик события
...
function forms(formselector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const formData = new FormData(form); //превращаем формдату в массив массивов, потом в классический обьект, а потом в json. Аналог перебора обьекта в обьект.

      const json = JSON.stringify(Object.fromEntries(formData.entries())); //это ссылка с реквестами с json-server. отправляем наш json на сервер
...
function slider(_ref) {
  ...
  let slideIndex = 1; //перменная которая показывает сколько мы отступили 

  let offset = 0; //условия по общему показу колва слайдеров
  ...
  slidesField.style.width = 100 * slides.length + '%'; //формируем в одну линию картинки

  slidesField.style.display = 'flex';
  slidesField.style.transition = '0.5s all'; //убираем с зоны видимости элементы за пределами нашего элемента.

  slidesWrapper.style.overflow = 'hidden'; //делаем все слайды одинаковой ширины 

  slides.forEach(slide => {
    slide.style.width = width;
  }); //теперь все абсолютно спозиционированные элементы будут ок

  slider.style.position = 'relative'; //создаем и стилизуем большую обертку для точек

  const indicators = document.createElement('ol'),
        dots = []; //добавляем класс, которого нет в css для себя
  ...
  slider.append(indicators); //цикл закончится когда закончатся слайды

  for (let i = 0; i < slides.length; i++) {
    //создаем точку как лист айтем
    const dot = document.createElement('li'); //добавляем каждой точке атрибут с нумерацией, начиная с 1 
  ...
  next.addEventListener('click', () => {
    ...
    slidesField.style.transform = `translateX(-${offset}px)`; // тут меняем слайдиндекс 
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_0__["default"])('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active'); //сделали это что бы не привязываться жестко к обьектам, иначе будут баги.
